...
```bash
# 1. 全銘柄の価格データを取得（初回フル取得：約15分、日次差分取得：約6分、約4400銘柄）
...
`eval_walkforward.py` によるウォークフォワード検証（2025-09 ~ 2026-02, 111営業日）:

| 戦略 | 正解率 |
|------|--------|
| **統合戦略 TOP1** | **81.1%**（111日中90日正解） |
| 値下がり予測 単独 TOP1 | 80.2% |
| 上昇予測 単独 TOP1 | 49.5% |

| | TOP1 | TOP3 | TOP5 |
|--|------|------|------|
| 値下がり予測 | 80.2% | 73.9% | 72.8% |
| 上昇予測 | 49.5% | 46.5% | 49.0% |

- 値下がり予測が主力。ほぼ毎日 DOWN 方向が採用される
- 上昇予測はランダム同等（49.5%）。精度改善は今後の課題
- テスト開始が2025-09なのは `TRAIN_LOOKBACK_YEARS=3` により学習データが直近3年に絞られるため

### Baseline vs Enhanced 比較（eval_compare.py）
...
Enhanced モードは値下がり単独で +2.2pp 改善。設定方法は「Enhanced モード」セクションを参照。
...
### 第4弾: ダウンロード高速化（実測6.7倍速）
- **バッチサイズ拡大**: `YFINANCE_BATCH_SIZE = 300`（`threads=False`化でスレッド競合が解消済みのため100→300に拡大。45バッチ→15バッチ）
- **並列ダウンロード**: `max_workers = 3`（各`yf.download(threads=False)`は独立した同期処理のため外部並列呼び出しは安全）
- **効果**: 45バッチ×逐次 → 15バッチ×3並列 = 5ラウンド。実測で39分 → 6分（**6.7倍速**）
